...
class Data:
    ...
    def get_data_list(self):
        ...
        """
        if "\n" in self.__data:
            data_clean = self.__data.replace('\t',' ').strip()
    ...
    def generate_new_list(temp_list,word):
        ...
        list_to_str = " "+str("  ".join(temp_list))+" "#2
        list_to_str = str(re.sub(r'(\s'+word+'\s)+',"",list_to_str)).strip()#2
        if not len(list_to_str) == 0: #1
            temp_list = list_to_str.replace("  ",",").replace(" ","").split(",")#3
        else:
            temp_list = []
        return temp_list
    ...
    def sorted_values(dic):
        def particion(lista, izq, der):
            ...
            for i in range(izq, der):
                if dic[lista[i]] >= pivote:#1
                    lista[indice], lista[i] = lista[i], lista[indice] #1
                    indice += 1#1
